...
public final class DatabaseAdminBackendHandlerFactory {
    ...
    public static Optional<TextProtocolBackendHandler> newInstance(final DatabaseType databaseType, final SQLStatementContext<?> sqlStatementContext,
        ...
        Optional<DatabaseAdminExecutor> executor = executorFactory.get().newInstance(sqlStatementContext, sql, Optional.ofNullable(connectionSession.getSchemaName()));
        return executor.map(optional -> createTextProtocolBackendHandler(sqlStatementContext, connectionSession, optional));
    }
    
    private static TextProtocolBackendHandler createTextProtocolBackendHandler(final SQLStatementContext<?> sqlStatementContext, final ConnectionSession connectionSession, final DatabaseAdminExecutor executor) {
        if (executor instanceof DatabaseAdminQueryExecutor) {
            return new DatabaseAdminQueryBackendHandler(connectionSession, sqlStatementContext, (DatabaseAdminQueryExecutor) executor);
        }
        if (executor instanceof DatabaseSetCharsetExecutor) {
            return new DatabaseSetCharsetBackendHandler(connectionSession, sqlStatementContext, (DatabaseSetCharsetExecutor) executor);
        }
        return new DatabaseAdminUpdateBackendHandler(connectionSession, sqlStatementContext, executor);
    }
}
